...
export class Store {
  ...
  @observable viewType = 'default';

  setModules(chunks) {
    for (const [, chunk] of Object.entries(chunks)) {
      walkModules(chunk, (module) => {
        // eslint-disable-next-line no-plusplus
        module.cid = this.cid++;
      });
    }

    this.rawChunks = chunks;
    this.selectedChunks = this.rawChunks[this.viewType];
  }

  @computed get allChunks() {
    console.log('target', this.rawChunks[this.viewType]);
    return this.rawChunks[this.viewType];
  }
  ...
  @computed get foundModulesByChunk() {
    ...
    return this.visibleChunks
      .map((chunk) => {
        ...
        foundGroups.forEach((modules) => modules.sort((m1, m2) => m2[activeSize] - m1[activeSize]));
